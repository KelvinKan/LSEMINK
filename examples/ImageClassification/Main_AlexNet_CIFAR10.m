...
if save_figures
    figure(1)
    figure_name = strcat('MLR(AlexNet)_training_', dataset, 'alpha=', num2str(options.alpha), '.tex');
    matlab2tikz(figure_name)

    figure(2)
    figure_name = strcat('MLR(AlexNet)_grad_', dataset, 'alpha=', num2str(options.alpha), '.tex');
    matlab2tikz(figure_name)

    figure(3)
    figure_name = strcat('MLR(AlexNet)_train_acc_', dataset, 'alpha=', num2str(options.alpha), '.tex');
    matlab2tikz(figure_name)

    figure(4)
    figure_name = strcat('MLR(AlexNet)_test_acc_', dataset, 'alpha=', num2str(options.alpha), '.tex');
    matlab2tikz(figure_name)
end